...
1. **_setup** — Verify OHLCV across all TFs (W, D, 4H, 1H, 15m, 5m), check indicator registry (RSI, MACD, ATR, VWAP, EMAs), auto-add missing indicators, acquire per-TF data, establish time-based context (Kill Zones, Opening Range, Session Bias)
...
6. **_momentum** — RSI/MACD/ADX, divergence (regular + hidden), EMA alignment, volatility regime (ATR)
7. **_confluence** — Score all inputs (weighted model), evaluate liquidity, determine decision: EXECUTE | WAIT | NO_TRADE | FLAG_ONLY
8. **_sizing** — Risk model (S+ 1%, A+ 0.75%, A 0.5%, B 0.25%, below 0.1%), stop beyond liquidity sweep or 1 ATR
